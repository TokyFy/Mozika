...
function MusicCard({title, artist, album, picture, file , isPlaying , isPaused}: IMusicCard) {
    return <>
        <div className={`group/mc flex gap-2 text-sm items-center px-1 p-1 max-w-lg cursor-pointer duration-500 ease-in rounded-sm hover:bg-neutral-200 ${isPlaying ? "bg-neutral-200" : ""}`}>
            <div>
                <div
            ...
            <div className="overflow-hidden">
                ...
                <p className="overflow-ellipsis text-neutral-500 whitespace-nowrap text-xs overflow-hidden first-letter:uppercase">{artist}</p>
            </div>
            <div className={`ml-auto items-center justify-center p-2 text-neutral-900 flex ${isPlaying ? "opacity-100" : "opacity-0"}`}>
                <Disc3 className={`${!isPaused ? "animate-spin" : ""}`} size={18}/>
            </div>
        </div>
    </>
}
